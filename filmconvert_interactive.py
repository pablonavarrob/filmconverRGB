...
import matplotlib
from border_selector import select_border_width
...
def white_balance(img, border_mask):
    """Apply white balance to a 16-bit image using simple scaling."""
    central_region = img[~border_mask]
    if central_region.ndim != 2 or central_region.shape[1] != 3:
        raise ValueError("Central region is not a 3-channel image.")

    avgR, avgG, avgB = np.mean(central_region, axis=0)
    kR = avgG / avgR
    kB = avgG / avgB
...
def balance_border(image, border_mask, display_edges=True):
    def find_dynamic_edge_values(image, border_mask):
        central_region = image[~border_mask]
        central_brightest_value = np.percentile(central_region, 85)
        border_values = image[border_mask]

        valid_border_values = border_values[
            (central_brightest_value <= border_values)
            & (border_values < np.iinfo(image.dtype).max - 10)
        ]
    ...
    edge_values = []
    for i in range(3):
        border_values = image[..., i][border_mask]
        edge_value = (
            np.median(border_values)
            if border_values.size
            else np.percentile(image[..., i][~border_mask], 95)
        )
        edge_values.append(edge_value)

    balanced_image = np.zeros_like(image)
    for i in range(3):
        balanced_image[..., i] = np.clip(
            image[..., i] * (np.mean(edge_values) / edge_values[i]), 0, 65535
        )

    if display_edges:
        edge_image = np.zeros_like(image[..., 0])
        edge_image[border_mask] = 65535
...
def contrast_stretch(image, border_mask):
...
def crop_image(image, angle, border_mask):
    """Crop the image by rotating to the given angle and then cropping the mask area."""
    rotated_image = rotate(image, angle, reshape=False, mode="constant", cval=0)
    return rotated_image[~border_mask]
...
def process_single_raw_file(image_path, operations, angle, border_mask, crop):
    ...
    if operations.get("color_balance"):
        ...
        image = white_balance(image, border_mask)
        print("Balancing black points...")
        image = balance_border(image, border_mask)

    if operations.get("invert"):
        print("Inverting image...")
        image = invert_image(image)

    image = contrast_stretch(image, border_mask)
    ...
    if crop:
        image = crop_image(image, angle, border_mask)

    print(
        f"Final image dtype: {image.dtype}, min: {np.min(image)}, max: {np.max(image)}"
    )
    output_path = os.path.join(
        os.path.dirname(image_path),
        f'{os.path.basename(image_path).split(".")[0]}_final.tiff',
    )
    imageio.imsave(output_path, image.astype("uint16"))
    print(f"Final image saved to {output_path}")


def process_single_tiff_file(image_path, operations, angle, border_mask, crop):
...
def process_image_set(
    path_red, path_green, path_blue, operations, angle, border_mask, crop
):
    ...
    if operations.get("color_balance"):
        print("Applying white balance...")
        merged_image = white_balance(merged_image, border_mask)
        print("Balancing black points...")
        merged_image = balance_border(merged_image, border_mask)

    if operations.get("invert"):
        print("Inverting image...")
        merged_image = invert_image(merged_image)

    image = contrast_stretch(merged_image, border_mask)
...
def process_bw_film(image_path, angle, border_mask, crop):
    ...
    final_image = contrast_stretch(bw_image, border_mask)
...
def process_folder(folder_path, capture_type, operations, angle, border_mask, crop):
    ...
    if capture_type == "rgb":
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            futures = [
                executor.submit(
                    process_triplet, triplet, operations, angle, border_mask, crop
                )
...
def process_triplet(triplet, operations, angle, border_mask, crop):
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""Examples:
    python script.py -m file -r red.tiff -g green.tiff -b blue.tiff -ct rgb --merge --color_balance --invert --crop
    python script.py -m folder -f /path/to/folder -ct i --merge --color_balance --invert --crop
    python script.py -m file -r bw_film.tiff -ct bw --crop
    """,
    )
    parser.add_argument(
        "-m",
    ...
    if args.mode == "file":
        if args.capture_type == "rgb":
            if not (args.red and args.green and args.blue):
                ...
                sys.exit(1)
            print("Selecting border width and rotation angle...")
            angle, border_mask = select_border_width(
                args.red
            ...
            if not args.red:
                ...
                sys.exit(1)
            print("Selecting border width and rotation angle...")
            angle, border_mask = select_border_width(args.red)  # Call this function
            # Call this function once
            if is_raw_file(args.red):
                process_single_raw_file(
                    args.red, operations, angle, border_mask, args.crop
                )
            ...
            process_bw_film(args.red, angle, border_mask, args.crop)
        ...
        print("Selecting border width and rotation angle...")
        first_image_path = os.path.join(args.folder, sorted(os.listdir(args.folder))[0])
        angle, border_mask = select_border_width(
            first_image_path
        )  # Call this function once

        process_folder(
            args.folder, args.capture_type, operations, angle, border_mask, args.crop
        )
